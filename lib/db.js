...
let dbPath = path.resolve(process.cwd(), 'apiron.db');

// If running on Vercel or in production, use the writeable /tmp directory to avoid read-only filesystem crashes on mutations
if (process.env.VERCEL === '1' || process.env.NODE_ENV === 'production') {
  const tmpPath = path.join('/tmp', 'apiron.db');
  try {
    if (!fs.existsSync(tmpPath)) {
      const srcPath = path.resolve(process.cwd(), 'apiron.db');
      if (fs.existsSync(srcPath)) {
        fs.copyFileSync(srcPath, tmpPath);
        console.log('Database successfully initialized in writeable location: /tmp/apiron.db');
      }
    }
    dbPath = tmpPath;
  } catch (err) {
...
// Alter existing tables to add file_data column for storing file contents in database
["pdfs", "images"].forEach((table) => {
  try {
    db.exec(`ALTER TABLE ${table} ADD COLUMN file_data BLOB`);
  } catch (e) {
    // Column already exists or table is new
  }
});
